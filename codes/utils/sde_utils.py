...
class DenoisingSDE(SDE):
    ...
    def _initialize(self, max_sigma, T, schedule, eps=0.04):
        ...
        def sigmoid_beta_schedule(timesteps, range = 2.5):
            timesteps = timesteps + 3    
            x = torch.linspace(-range, range, timesteps, dtype=torch.float32)  
            alphas_cumprod = torch.sigmoid(x) ** 2  
            alphas_cumprod = alphas_cumprod / alphas_cumprod[-1]  
            betas = alphas_cumprod[1:-1]  
            return betas

        def pow_beta_schedule(timesteps):
            timesteps = timesteps + 2
            steps = timesteps + 1
            x = torch.linspace(0, timesteps, steps, dtype = torch.float32)
            alphas_cumprod = torch.pow(0.85,x)
            betas = 1 - alphas_cumprod[1:-1]
            return betas
        ...
        if schedule == 'cosine':
            ...
            print('sigmoid')
            thetas = sigmoid_beta_schedule(T)
        elif schedule == 'pow':
            print('pow')
            thetas = pow_beta_schedule(T)
        else:
            print('linear')
    ...
    def generate_random_states(self, x0):
        ...
        return timesteps, noisy_states, noises * noise_level
